const STORAGE_KEYS = {
  ...
  settingsVersion: 'restaurant_settings_version',
};
...
function mergeSettings() {
  const storedVersion = parseInt(localStorage.getItem(STORAGE_KEYS.settingsVersion) || '0', 10);
  if (storedVersion >= SETTINGS_VERSION) return;

  const current = JSON.parse(localStorage.getItem(STORAGE_KEYS.settings) || '{}');
  const merged = { ...DEFAULT_SETTINGS, ...current };

  if (storedVersion < 2) {
    merged.upiId = DEFAULT_SETTINGS.upiId;
  }
...
function saveMenu(menu) {
  ...
  localStorage.setItem(STORAGE_KEYS.menuVersion, String(Date.now()));
}
...
function saveSettings(settings) {
  ...
  localStorage.setItem(STORAGE_KEYS.settingsVersion, String(Date.now()));
}
...
function menuImageUrl(image) {
  ...
  const v = localStorage.getItem(STORAGE_KEYS.menuVersion) || MENU_VERSION;
...
document.addEventListener('DOMContentLoaded', () => {
  if (typeof loadRemoteSiteData === 'function') {
    loadRemoteSiteData();
  }
});
